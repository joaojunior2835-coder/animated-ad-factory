...
export default function StudioWorkbench({ sessionId, studio, onUpdate, onCreateSession, onAdvanced, onSessions, productLibrary = [], context, onContextChange, onOpenCreative, onOpenCanvas, onCreateVariation }) {
  ...
  function chooseModel(id) {
    ...
    if (production.mediaKind === 'image') {
      ...
      setNotice('Image model changed. Ask for a fresh quote before generation.')
      return
    }
  ...
  async function ensureSaved() {
    ...
    const draftScenes = scenes.length ? scenes : [{ id: `prompt-${Date.now()}`, name: production.mediaKind === 'image' ? 'Image concept' : 'Scene 1', visualDescription: studio.brief.hook, script: '', scriptLocked: false, seconds: model?.durations?.includes(5) ? 5 : model?.durations?.[0] || 5 }]
