...
export default function StudioWorkbench({ sessionId, studio, onUpdate, onCreateSession, onAdvanced, onSessions, productLibrary = [], context, onContextChange, onOpenCreative, onOpenCanvas, onCreateVariation }) {
  ...
  const models = (options?.capabilities || []).filter(c => c.capability === 'generate_video' && c.configured && c.priced)
  ...
  const inputKey = JSON.stringify({ product: studio.product, brief: studio.brief, ...production, creativeId: null, productTestId: null, newCreative: false, sceneLinks: {} })
  ...
  function chooseModel(id) {
    ...
    setNotice(reset ? 'Les réglages incompatibles ont été adaptés au modèle. Les scripts et la photo sont conservés; une nouvelle estimation est nécessaire.' : 'Modèle modifié. Une nouvelle estimation est nécessaire.')
  }

  async function ensureSaved() {
    if (!sessionId) throw new Error('Créez un brouillon Studio avant la production.')
    if (!scenes.length) throw new Error('Proposez au moins une scène.')
    ...
    const source = { ...studio, production: { ...production, modelId: model?.id || production.modelId } }
    const compiled = scenes.map(s => buildStudioGenerationScene(s, source, model))
    let id = creativeId
    if (!id) {
      if (!productTestId) throw new Error('Choisissez un Product Test dans le contexte de production. Aucun budget n’est créé automatiquement.')
      const result = await request('creatives', { productTestId: Number(productTestId), angle: `${studio.product.name} · ${STUDIO_PRESETS.find(t => t.id === production.templateId)?.name || 'Studio'}`, marketingStudioSessionId: sessionId })
      id = result.creativeId
      // Persist the new lineage before subsequent preparation can fail; retries reuse it.
      ...
      onUpdate(s => ({ ...s, production: { ...normalizeStudioProduction(s.production), creativeId: Number(id), newCreative: false, productTestId: Number(productTestId), sceneLinks: {} } }))
      onContextChange?.({ productTestId: Number(productTestId), creativeId: Number(id) })
    }
    const loaded = (await request(String(id))).workspace
    const prepared = prepareStudioSceneSave(loaded, scenes, compiled, production.creativeId === Number(id) || !production.creativeId ? production.sceneLinks : {})
    ...
    const sceneLinks = Object.fromEntries(scenes.map((s, i) => [s.id, saved.scenes[prepared.indices[i]].id]))
    onUpdate(s => ({ ...s, production: { ...normalizeStudioProduction(s.production), creativeId: Number(id), productTestId: productTestId || production.productTestId, sceneLinks } }))
    onContextChange?.({ productTestId: productTestId || production.productTestId, creativeId: Number(id) })
    setWork(saved)
    return { id, workspace: saved, sceneLinks }
  }

  const prepareQuote = selectedDraftId => act(async () => {
    const result = await ensureSaved()
    const scope = scenes.filter(s => !selectedDraftId || s.id === selectedDraftId).map(s => result.workspace.scenes.find(v => v.id === result.sceneLinks[s.id])).filter(s => selectedDraftId || !(s.status === 'Complete' && s.current))
    ...
    setQuote({ ...response.quote, creativeId: result.id, inputKey, scope, label: model.label, resolution: production.resolution, aspectRatio: production.aspectRatio, audio: production.generateAudio })
  })
  ...
  return <section className="studio-workbench" data-testid="studio-workbench">
    <header className="sw-header"><div><span className="sw-eyebrow">MARKETING STUDIO</span><h2>Votre produit. Vos prochaines idées.</h2></div><div className="sw-actions">{onSessions && <button className="ghost" onClick={onSessions}>Sessions</button>}<button className="ghost" onClick={onAdvanced}>Advanced · formats & exports</button></div></header>
    ...
    <div className="sw-layout"><aside className="sw-product"><div className="sw-product-photo">{photo ? <img src={mediaUrl(photo)} alt={`Photo de ${studio.product.name || 'votre produit'}`} /> : <div><span>Votre produit</span><small>Ajoutez sa photo</small></div>}<button className="ghost small" disabled={!sessionId || readonly} onClick={() => uploadRef.current?.click()}>Importer une photo</button><input ref={uploadRef} type="file" hidden accept="image/png,image/jpeg,image/webp" onChange={e => { upload(e.target.files[0]); e.target.value = '' }} /></div>
      ...
      <div className="sw-gallery-heading"><h3>Choisir une direction</h3><div className="sw-filters" aria-label="Catégories de presets">{['Tous', 'Produit', 'Lifestyle', 'Court'].map(c => <button key={c} aria-pressed={category === c} onClick={() => setCategory(c)}>{c}</button>)}</div></div>
      <div className="sw-presets">{STUDIO_PRESETS.filter(p => category === 'Tous' || p.category === category).map(p => <button key={p.id} className={`sw-preset${production.templateId === p.id ? ' selected' : ''}`} aria-pressed={production.templateId === p.id} disabled={readonly || !sessionId} onClick={() => { patch({ templateId: p.id, ...(!scenes.length ? { scenes: proposeStudioScenes(p.id, studio.brief.language) } : {}) }); if (scenes.length) setNotice('Direction sélectionnée. Vos scènes existantes sont conservées. Créez une variation pour une nouvelle proposition.') }}><PresetArt preset={p} photo={photo} /><strong>{p.name}</strong><small>{p.description}</small></button>)}</div>
      ...
      <div className="sw-composer"><label>Brief créatif<textarea aria-label="Brief créatif Studio" rows={2} disabled={!sessionId || readonly} maxLength={1200} value={studio.brief.hook} onChange={e => update(s => ({ ...s, brief: { ...s.brief, hook: e.target.value } }))} placeholder="La direction, l’ambiance ou l’idée à tester…" /></label><div className="sw-composer-bottom"><div className="sw-settings"><label>Modèle<select aria-label="Modèle Studio" disabled={readonly || !models.length} value={model?.id || ''} onChange={e => chooseModel(e.target.value)}>{!model && <option value="">Aucun modèle configuré</option>}{models.map(m => <option key={m.id} value={m.id}>{m.label}</option>)}</select></label><label>Format<select aria-label="Format Studio" disabled={readonly || !model} value={production.aspectRatio} onChange={e => patch({ aspectRatio: e.target.value })}>{!model?.aspects.includes(production.aspectRatio) && <option>{production.aspectRatio}</option>}{model?.aspects.map(a => <option key={a}>{a}</option>)}</select></label><label>Résolution<select aria-label="Résolution Studio" disabled={readonly || !model} value={production.resolution} onChange={e => patch({ resolution: e.target.value })}>{!model?.resolutions.includes(production.resolution) && <option>{production.resolution}</option>}{model?.resolutions.map(r => <option key={r}>{r}</option>)}</select></label><label className="sw-check"><input type="checkbox" checked={production.generateAudio} disabled={readonly || !model?.audio} onChange={e => patch({ generateAudio: e.target.checked })} />Audio</label><span className="sw-small">1 sortie / scène</span></div><button className="primary sw-generate" disabled={!sessionId || readonly || !model || !scenes.length || hasRunning} onClick={() => prepareQuote()}>Générer les scènes <span>{money(estimate)}</span></button></div><p className="sw-small">{production.productAssetId ? 'Image → vidéo' : 'Texte → vidéo'} · Proposition locale gratuite · Coût total affiché avant confirmation · Aucun effet sur le prix promis pour l’audio désactivé</p></div>
