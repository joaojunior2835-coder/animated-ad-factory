...
const CreativeGenerator = forwardRef(function CreativeGenerator({ studio, onReview, onProductTests, context, onContextChange, workspace='create_ad' }, ref) {
  ...
  const [selectedVideoScene, setSelectedVideoScene] = useState(0)
  const [videoHistoryView, setVideoHistoryView] = useState('list')
  const [videoReferenceTab, setVideoReferenceTab] = useState('references')
  ...
  const videoSceneIndex = Math.min(selectedVideoScene, Math.max(0, scenes.length - 1))
  const currentVideoDraft = scenes[videoSceneIndex] || blank()
  const videoPrompt = currentVideoDraft.prompt || brief
  ...
  const selectedVideoLive = work?.scenes.find(s => s.id === currentVideoDraft.id)
  const selectedVideoPrice = estimate?.rows.find(r => r.sceneId === currentVideoDraft.id)
  const videoResults = (work?.scenes || []).filter(s => s.media).slice().reverse()
  ...
  const setVideoPrompt = value => { setBrief(value); if (scenes.length <= 1) ensureOneVideoScene(value); else change(videoSceneIndex, 'prompt', value) }
  ...
  const hasVideoMedia = work?.scenes?.some(s => s.media)
  const remixIndex = scenes.findIndex(s => s.remix)
  const remixScene = remixIndex >= 0 ? scenes[remixIndex] : null
  ...
  const showFinalAssembly = ((workspace !== 'video' && workspace !== 'remix' && entry !== 'remix') || (workspace === 'video' && scenes.length > 1) || work?.final?.finalAsset) && !showRemixPrimary
  return <section className={`generator cg-workspace-${workspace} ${quickMode ? 'cg-quick-mode' : ''}`} data-testid="creative-generator">
    <header className="cg-header"><div><span className="cg-eyebrow">CREATE</span><h2>{workspace==='remix'?'Remix':workspace==='video'?'Video':'Create Ad'}</h2>{workspace==='remix'?<p>Reference video + reference images + one instruction.</p>:null}</div>{quickMode?<button className="ghost cg-new-compact" disabled={busy || quickCreating.current} onClick={resetQuickDraft}>{workspace==='remix'?'New remix draft':'New'}</button>:<button className="ghost" onClick={onProductTests}>Product setup</button>}</header>
    ...
    {!options ? <p>Loading your workspace…</p> : <>
      {workspace==='video'&&work&&<section className="cg-video-primary" aria-label="Video creation composer"><div className="cg-video-stage">{work.scenes.some(s=>s.media)?<div className="cg-video-history">{work.scenes.filter(s=>s.media).slice(-6).map((s,i)=><div key={`${s.id}-${i}`}><VideoPreview asset={s.media} compact maxWidth={520}/><small>{s.name}</small></div>)}</div>:<div className="cg-placeholder">What do you want to create?</div>}</div><div className="cg-video-composer"><div className="cg-attach-row"><details><summary aria-label="Add video attachment">+</summary><div className="cg-attach-menu"><label className="field">Upload image<input type="file" accept="image/png,image/jpeg,image/webp" disabled={busy} onChange={e=>upload(0,e.target.files[0])}/></label><label className="field">Choose from Assets<select aria-label="Video composer start frame" disabled={busy} value={currentVideoDraft.startAssetId||''} onChange={e=>change(0,'startAssetId',Number(e.target.value)||null)}><option value="">No start frame</option>{options.media.filter(a=>a.mime_type.startsWith('image/')).map(a=><option key={a.id} value={a.id}>{a.relative_path.split('/').pop()}</option>)}</select></label><details><summary>Add context</summary><div className="cg-selectors-link"><label className="field">Product<select aria-label="Product Test" value={productId} disabled={busy || dirty} onChange={e => { setProductId(e.target.value); setCreativeId('') }}><option value="">No Product Test</option>{options.productTests.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label className="field">Creative<select aria-label="Creative" value={creativeId} disabled={busy || dirty} onChange={e => { setCreativeId(e.target.value); sessionStorage.setItem(`quick-generator:${workspace}:${apiBase()}`, JSON.stringify({ productTestId: productId, creativeId: e.target.value, linked: true })) }}><option value="">No linked Creative</option>{options.creatives.filter(c => String(c.product_test_id) === productId).map(c=><option key={c.id} value={c.id}>{c.angle}</option>)}</select></label></div></details></div></details>{videoStartAsset&&<span className="cg-reference-pill"><img src={mediaUrl(videoStartAsset)} alt="Attached start frame"/>Start frame <button type="button" className="ghost small" onClick={()=>change(0,'startAssetId',null)}>×</button></span>}</div><label className="field cg-video-prompt">Describe the video you want<textarea aria-label="Video prompt" value={videoPrompt} disabled={busy} rows={2} placeholder="Slow handheld camera push toward a red apple on a white table, 4 seconds." onChange={e=>setVideoPrompt(e.target.value)}/></label><div className="cg-video-controls"><label className="field cg-structure">Structure<select aria-label="Video structure" value={videoMode} onChange={e=>setVideoMode(e.target.value)} disabled={busy}><option value="auto">Auto</option><option value="single">Single clip</option><option value="multi">Multi-scene</option></select></label><label className="field">Model<select aria-label="Video model" disabled={busy} value={currentVideoDraft.provider} onChange={e=>change(0,'provider',e.target.value)}>{options.models.map(m=><option key={m.id} value={m.id}>{m.label} · {m.providerLabel}</option>)}</select></label><ModelSettings model={videoCapability} value={currentVideoDraft} onChange={(key,value)=>change(0,key,value)} disabled={busy} prefix="Video"/><button className="primary cg-video-generate" disabled={busy || !videoPrompt.trim()} onClick={videoPrimaryGenerate}>{scenes.length>1?'Generate all':'Generate'}<small>{dirty?'Quote before confirmation':money(estimate?.totalMinor)}</small></button></div></div></section>}
      {workspace==='video' && work && <section className="video-generator" aria-label="Video generator">
        <aside className="video-generator__rail">
          <div className="video-generator__rail-heading"><span className="cg-eyebrow">CREATE VIDEO</span><div className="video-generator__tabs"><button className="active" type="button">Create Video</button></div></div>
          <div className="video-generator__references">
            <div className="video-generator__reference-tabs"><button className={videoReferenceTab==='references'?'active':''} type="button" onClick={()=>setVideoReferenceTab('references')}>References</button><button className={videoReferenceTab==='frames'?'active':''} type="button" onClick={()=>setVideoReferenceTab('frames')} disabled={!videoStartAsset} title={!videoStartAsset ? 'Add a start image to use Frames.' : undefined}>Frames</button></div>
            {videoReferenceTab==='references' ? <><details className="video-generator__attach"><summary>+ Add references <small>Image or Video</small></summary><div className="video-generator__attach-menu"><label>Upload image<input type="file" accept="image/png,image/jpeg,image/webp" disabled={busy} onChange={e=>upload(videoSceneIndex,e.target.files[0])}/></label><label>Choose from Assets<select aria-label="Video reference asset" disabled={busy} value={currentVideoDraft.startAssetId||''} onChange={e=>change(videoSceneIndex,'startAssetId',Number(e.target.value)||null)}><option value="">Choose an image</option>{options.media.filter(a=>a.mime_type.startsWith('image/')).map(a=><option key={a.id} value={a.id}>{a.relative_path.split('/').pop()}</option>)}</select></label><p>For a reference video, use the existing Remix workspace.</p></div></details>{videoStartAsset ? <div className="video-generator__reference"><img src={mediaUrl(videoStartAsset)} alt="Selected product reference"/><span>Start frame</span><button className="ghost small" type="button" onClick={()=>change(videoSceneIndex,'startAssetId',null)}>Remove</button></div> : <p className="video-generator__reference-hint">Attach a product image to make image-to-video available.</p>}</> : <div className="video-generator__frame"><img src={mediaUrl(videoStartAsset)} alt="Selected start frame"/><strong>Start Frame</strong><small>End frame is not supported by the selected model.</small></div>}
          </div>
          <label className="video-generator__prompt">Prompt<textarea aria-label="Video prompt" value={videoPrompt} disabled={busy} placeholder="Describe your scene in detail..." onChange={e=>setVideoPrompt(e.target.value)}/></label>
          <label className="video-generator__model">Model<select aria-label="Video model" disabled={busy} value={currentVideoDraft.provider} onChange={e=>change(videoSceneIndex,'provider',e.target.value)}>{options.models.map(m=><option key={m.id} value={m.id}>{m.label} · {m.providerLabel}</option>)}</select></label>
          <div className="video-generator__chips"><label><span>Duration</span><select aria-label="Video duration" value={currentVideoDraft.seconds} disabled={busy} onChange={e=>change(videoSceneIndex,'seconds',Number(e.target.value))}>{videoCapability?.durations?.map(v=><option key={v} value={v}>{v}s</option>)}</select></label><label><span>Aspect</span><select aria-label="Video aspect ratio" value={currentVideoDraft.aspectRatio} disabled={busy} onChange={e=>change(videoSceneIndex,'aspectRatio',e.target.value)}>{videoCapability?.aspects?.map(v=><option key={v} value={v}>{v}</option>)}</select></label><label><span>Resolution</span><select aria-label="Video resolution" value={currentVideoDraft.resolution} disabled={busy} onChange={e=>change(videoSceneIndex,'resolution',e.target.value)}>{videoCapability?.resolutions?.map(v=><option key={v} value={v}>{v}</option>)}</select></label>{videoCapability?.audio&&<label><span>Audio</span><select aria-label="Video audio" value={currentVideoDraft.generateAudio?'on':'off'} disabled={busy} onChange={e=>change(videoSceneIndex,'generateAudio',e.target.value==='on')}><option value="off">Audio off</option><option value="on">Audio on</option></select></label>}</div>
          <details className="video-generator__more"><summary>More settings</summary><label>Structure<select aria-label="Video structure" value={videoMode} onChange={e=>setVideoMode(e.target.value)} disabled={busy}><option value="auto">Auto</option><option value="single">Single clip</option><option value="multi">Multi-scene</option></select></label></details>
          <button className="primary video-generator__generate" disabled={busy || !videoPrompt.trim()} onClick={videoPrimaryGenerate}>{busy ? 'Generating…' : scenes.length>1 ? 'Generate all' : 'Generate'}<small>{dirty ? 'Estimating…' : money(scenes.length>1 ? estimate?.totalMinor : selectedVideoPrice?.minor ?? estimate?.totalMinor)}</small></button>
        </aside>
        <main className="video-generator__media">
          <header><div><span className="cg-eyebrow">VIDEO</span><h3>History</h3></div><div className="video-generator__view-toggle"><button className={videoHistoryView==='list'?'active':''} onClick={()=>setVideoHistoryView('list')}>List</button><button className={videoHistoryView==='grid'?'active':''} onClick={()=>setVideoHistoryView('grid')}>Grid</button></div></header>
          {scenes.length>1&&<div className="video-generator__scenes" aria-label="Ad scenes">{scenes.map((s,i)=>{const live=work.scenes.find(v=>v.id===s.id);return <button key={s.id||i} className={i===videoSceneIndex?'active':''} onClick={()=>setSelectedVideoScene(i)}><b>{String(i+1).padStart(2,'0')}</b><span>{s.name || `Scene ${i+1}`}</span><small>{s.seconds}s · {live?.displayStatus||live?.status||'Ready'}</small></button>})}</div>}
          {videoResults.length ? <div className={`video-generator__results ${videoHistoryView}`}>{videoResults.map((scene,i)=><button key={`${scene.id}-${scene.media.id}`} className={scene.id===selectedVideoLive?.id?'selected':''} onClick={()=>setSelectedVideoScene(scenes.findIndex(s=>s.id===scene.id))}><VideoPreview asset={scene.media} compact maxWidth={videoHistoryView==='grid'?340:780}/><span>{scene.name}</span><small>{scene.provider} · {scene.seconds}s · {scene.aspectRatio} · {scene.displayStatus||scene.status}</small></button>)}</div> : <div className="video-generator__empty"><p>Video results appear here.</p><span>References → prompt → settings → Generate</span></div>}
          {scenes.length>1&&approved>0&&<section className="video-generator__assembly"><span>{approved} scene{approved===1?'':'s'} ready</span><button className="primary" disabled={busy||dirty||!options.ffmpeg} onClick={()=>act(async()=>{const w=await save();const r=await api(`${creativeId}/assemble`,{revision:w.revision});accept(r.workspace);await refreshOptions()})}>Assemble final ad</button></section>}
        </main>
        <aside className="video-generator__inspector"><span className="cg-eyebrow">SELECTED GENERATION</span>{selectedVideoLive?.media ? <><VideoPreview asset={selectedVideoLive.media} compact maxWidth={260}/><h3>{options.models.find(m=>m.id===currentVideoDraft.provider)?.label || currentVideoDraft.provider}</h3><p>{currentVideoDraft.prompt || 'No prompt recorded.'}</p><div className="video-generator__meta"><span>{currentVideoDraft.resolution}</span><span>{currentVideoDraft.seconds}s</span><span>{currentVideoDraft.aspectRatio}</span><span>Audio {currentVideoDraft.generateAudio?'on':'off'}</span></div><div className="video-generator__inspector-actions"><button className="ghost" onClick={()=>download(selectedVideoLive.media)}>Download</button><button className="ghost" onClick={onReview}>Send to Review</button></div></> : <div className="video-generator__inspector-empty">Select a generated video to see its prompt, references and settings.</div>}</aside>
      </section>}
      ...
      {work && <>
        {showRemixPrimary ? <section className="cg-remix-primary" data-testid="generator-scene-1" aria-label="Remix reference ad workspace"><aside className="cg-remix-rail"><ReferenceRemix scene={remixScene} options={options} locked={remixLocked} creativeId={creativeId} onChange={next=>edit(scenes.map((v,n)=>n===remixIndex?next:v))} act={act} request={request} refreshOptions={refreshOptions} onSegments={()=>splitRemix(remixIndex)}/><div className="cg-remix-settings"><h4>Settings</h4><div className="cg-controls"><label className="field">Model<select aria-label="Remix model" disabled={remixLocked} value={remixScene.provider} onChange={e=>change(remixIndex,'provider',e.target.value)}>{!options.models.some(m=>m.id===remixScene.provider)&&<option value={remixScene.provider}>Unavailable — configure provider</option>}{options.models.map(m=><option key={m.id} value={m.id}>{m.label} · {m.providerLabel}</option>)}</select></label><ModelSettings model={options.capabilities.find(m=>m.capability==='generate_video'&&m.provider===remixScene.provider)} value={remixScene} onChange={(key,value)=>change(remixIndex,key,value)} disabled={remixLocked} prefix="Remix" reference /></div><p className="cg-remix-cost">Estimated cost: <strong>{dirty?'Updating…':(!remixScene.remix?.scriptApproved?'Add references and an instruction':money(remixPrice?.minor))}</strong>{remixPrice?.error&&remixScene.remix?.scriptApproved&&<span className="note bad"> {remixPrice.error}</span>}</p>{remixHeld?<div className="note bad" role="alert">Billing status uncertain — manual reconciliation required. No replacement generation is available here.</div>:<button className="primary cg-remix-generate" aria-label={remixLive?.media?'Regenerate remix':'Generate remix'} disabled={remixLocked||!generationReady(remixScene)} onClick={()=>askGenerate([remixIndex])}>{remixLive?.media?'Regenerate':'Generate'}<small>{dirty?'Quote before confirmation':money(estimate?.totalMinor)}</small></button>}<span className="cg-remix-save-status" role="status">{busy ? 'Working…' : dirty ? 'Saving edits…' : 'Saved locally'}</span></div></aside><main className="cg-remix-media"><RemixComparison original={options.media.find(a=>a.id===remixScene.remix?.sourceAssetId)} output={remixLive?.media} onDownload={download} busy={busy}/>{remixLive?.media?<div className="cg-remix-result-actions"><button className="ghost" disabled={remixLocked||dirty||!remixLive.current||remixLive.approved} onClick={()=>resultAction(remixIndex,{approved:true})}>Use as Scene</button><button className="ghost" disabled={remixLocked||dirty||!remixLive.current} onClick={()=>useRemixCreative(remixIndex)}>Use in Creative</button><button className="ghost" onClick={onReview}>Send to Review</button></div>:<p className="cg-remix-empty">Add a reference video, visual references, and one instruction. The remixed result appears here beside the original.</p>}<details className="cg-remix-history"><summary>History</summary><p>{remixLive?.history?.length||0} previous generation{(remixLive?.history?.length||0)===1?'':'s'} retained.</p>{remixLive?.details&&<pre>{JSON.stringify({attempts:remixLive.details.attempts,costs:remixLive.details.costs},null,2)}</pre>}</details></main></section> : null}
        {showSceneWorkbench ? <div className="cg-summary"><h3>{quickMode ? (workspace==='remix'?'Quick remix':'Quick video') : work.creative.angle}</h3>{!quickMode&&<p>{work.creative.concept_summary || work.creative.hook_text}</p>}<div className="cg-stats"><span>{scenes.length} scenes</span><span>{work.scenes.filter(s=>s.status==='Complete').length} complete</span><span>{approved} / {scenes.length} approved</span><span>Estimate: {money(estimate?.totalMinor)}</span>{!quickMode&&<><span>Recorded: {money(work.recordedMinor)}</span><span>Iteration budget: {money(work.budget.budgetCeilingMinor)}</span></>}</div>{!quickMode&&<small>Recorded costs may use catalog estimates, not verified provider invoices.</small>}</div> : null}
        ...
        {showSceneWorkbench&&!scenes.length&&<div className="cg-empty">{entry==='remix'?'Add a remix scene, drop a reference ad and choose what should change.':'Add a scene or generate an editable proposal to begin.'}</div>}
        {showFinalAssembly ? <section className="cg-final subpanel"><div className="cg-actions"><h3>Final Ad</h3><button className="primary" disabled={busy||dirty||!approved||!options.ffmpeg} onClick={()=>act(async()=>{const w=await save(); const r=await api(`${creativeId}/assemble`,{revision:w.revision});accept(r.workspace);await refreshOptions()})}>{busy?'Working…':'Assemble final ad'}</button><span>{approved} approved scene(s) · visible order · local / free</span></div>
