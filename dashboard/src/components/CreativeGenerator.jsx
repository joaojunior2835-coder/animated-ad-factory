...
import ReferenceRemix, { newRemix, RemixComparison } from './ReferenceRemix.jsx'
...
export default function CreativeGenerator({ studio, onReview, onProductTests }) {
  ...
  const ready = scenes.map((s,i) => ({s,i, live:work?.scenes.find(v => v.id === s.id)})).filter(({s,live}) => s.prompt.trim() && (!s.remix || s.remix.scriptApproved) && !['Queued','Generating','Reconciliation required','Complete'].includes(live?.status)).map(v => v.i)
