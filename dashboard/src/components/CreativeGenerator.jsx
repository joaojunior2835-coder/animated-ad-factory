...
const CreativeGenerator = forwardRef(function CreativeGenerator({ studio, onReview, onProductTests, context, onContextChange, workspace='create_ad' }, ref) {
  ...
  const showSceneWorkbench = workspace !== 'video' || scenes.length > 1 || scenes.some(s => s.remix) || hasVideoMedia
  const showFinalAssembly = workspace !== 'video' || scenes.length > 1 || hasVideoMedia || work?.final?.finalAsset
