...
export async function runVideoGeneration({ prompt, startFrameUrl = '', aspectRatio = '9:16', duration = 5, provider = DEFAULT_VIDEO_API_PROVIDER, replicateModel = 'ltx', pollIntervalMs = 10_000, maxWaitMs = 5 * 60_000, confirmCost, onConfirmed } = {}) {
  let started = await generateVideo(prompt, startFrameUrl, aspectRatio, duration, provider, false, replicateModel)
  if (provider === 'replicate' && started && started.error === 'confirmation_required') {
    ...
    started = await generateVideo(prompt, startFrameUrl, aspectRatio, duration, provider, true, replicateModel)
  }
