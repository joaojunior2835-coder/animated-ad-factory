...
try {
  ...
  await test('inline text preserves Delete and native typing',async()=>{
    ...
    await node('prompt-main').getByLabel('Prompt text',{exact:true}).fill('A factual bottle composition')
    const before=await page.locator('.gnode').count()
    await node('prompt-main').getByLabel('Prompt text',{exact:true}).press('Backspace')
  ...
  await test('model settings expose real size/quantity and reset shape consistently',async()=>{
    ...
    await node('image-main').getByLabel('Image size',{exact:true}).selectOption('portrait')
    assert.equal(await node('image-main').getByLabel('Aspect ratio',{exact:true}).inputValue(),'9:16')
    await node('image-main').getByLabel('Image outputs',{exact:true}).selectOption('2')
    await node('image-main').getByLabel('Image size',{exact:true}).selectOption('square')
    await node('image-main').getByLabel('Image outputs',{exact:true}).selectOption('1')
  })
  ...
  await test('image upload saves once and registers a stable Asset without copying the file',async()=>{
    ...
    await upload.locator('input[type=file]').setInputFiles({name:'upload.png',mimeType:'image/png',buffer:png})
    await upload.getByRole('button',{name:'Save to Library',exact:true}).click()
