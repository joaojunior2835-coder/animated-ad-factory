...
try {
  ...
  await check('Image → Video links the existing Asset, preserves image history, and never uploads or duplicates it', async () => {
    ...
    await generator().locator('.cg-reference-pill').waitFor()
    await until(async () => {
      const rows = db.prepare("SELECT key, value FROM app_settings WHERE key LIKE 'creative_generator_%'").all()
      for (const row of rows) {
        const state = JSON.parse(row.value)
    ...
    await generator().getByLabel('Video prompt', { exact: true }).fill('A slow camera movement around the same bottle. Preserve its visible shape and packaging.')
    await generator().locator('.cg-video-generate').click()
    await page.getByRole('dialog', { name: 'Confirm scene generation' }).getByRole('button', { name: 'Cancel', exact: true }).click()
    await until(async () => (await workspace(videoCreativeId)).scenes.find(scene => scene.id === videoSceneId)?.prompt?.includes('Preserve its visible shape'), 'Video prompt was not persisted')
  ...
  await check('Video confirms Mock generation, survives refresh and plays as a single-result surface', async () => {
    await generator().locator('.cg-video-generate').click()
    ...
    await page.reload()
    const videoCard = generator().locator('.cg-video-stage video').first()
    await videoCard.waitFor({ timeout: 60000 })
    const fixture = await play(videoCard, 'Video Mock actual metadata')
    ...
    assert.match(await generator().locator('.cg-video-stage').innerText(), new RegExp(`${fixture.duration.toFixed(2).replace('.', '\\.')} s`))
    assert.match(await generator().locator('.cg-video-stage').innerText(), /Local test fixture \(not an AI result\)/)
    assert.equal(await generator().getByTestId('generator-scene-1').count(), 0)
    assert.equal(await generator().getByRole('button', { name: 'Generate all ready scenes', exact: true }).count(), 0)
    await shot('video-desktop', generator().locator('.cg-video-stage'))
    await shot('video-composer-desktop', generator().locator('.cg-video-composer'))
  })
  ...
  await check('Canvas reviews and confirms only the selected image → video dependency path through M5', async () => {
    ...
    await page.locator(`.gnode[data-node-id="${imageNode.id}"]`).getByRole('button', { name: 'Edit properties', exact: true }).click()
    await page.locator('.nc-properties-side').getByLabel('Image size', { exact: true }).selectOption('square_hd')
    await page.locator(`.gnode[data-node-id="${videoNode.id}"]`).getByRole('button', { name: 'Edit properties', exact: true }).click()
    await page.locator('.nc-properties-side').getByLabel('Resolution', { exact: true }).selectOption('480p')
  ...
  await check('Mobile navigation, image composer, playable media, Studio and Assets fit at 390 × 844', async () => {
    ...
    await generator().locator('.cg-video-stage video').first().waitFor()
    await assertNoHorizontalOverflow('Mobile Video')
    await shot('video-mobile', generator().locator('.cg-video-stage'))
    await shot('video-composer-mobile', generator().locator('.cg-video-composer'))
    await navigate('marketing_studio')
    await studio().getByRole('button', { name: 'Advanced Studio', exact: true }).click()
    await studio().getByLabel('Modèle Studio').getByRole('option', { name: /Mock Video/ }).waitFor({ state: 'attached' })
