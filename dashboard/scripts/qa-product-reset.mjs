...
try {
  ...
  await check('Video prompt-first screen has no mandatory mode or testing context', async () => {
    ...
    await confirmGeneration(page)
    await page.locator('.cg-video-stage video').first().waitFor({ timeout: 20000 })
    await page.waitForFunction(() => {
      const video = document.querySelector('.cg-video-stage video')
      return video && video.videoWidth > 0 && video.videoHeight > 0
    }, null, { timeout: 10000 })
    const videoAspect = await page.locator('.cg-video-stage video').first().evaluate(video => ({ rendered: video.getBoundingClientRect().width / video.getBoundingClientRect().height, natural: video.videoWidth / video.videoHeight }))
