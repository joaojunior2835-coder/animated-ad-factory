...
try {
  ...
  await check('Image quick workflow needs no Product Test and preserves 1:1, 9:16, 16:9, and multi-output sizing', async () => {
    ...
    const first = await assertBox(page, squareCard.locator('.asset-preview'), box => box.width >= 240 && box.width <= 370 && Math.abs(box.width - box.height) < 24, '1:1 image output stays moderate and square')
    const second = await assertBox(page, portraitCard.locator('.asset-preview'), box => box.width >= 240 && box.width <= 370 && box.height > box.width * 1.35, '9:16 image output stays portrait')
    await assertBox(page, landscapeCard.locator('.asset-preview'), box => box.width >= 240 && box.width <= 370 && box.width > box.height * 1.35, '16:9 image output stays landscape')
    ...
    await assertBox(page, portraitCard.locator('.asset-preview'), box => box.width >= 240 && box.width <= 370 && box.height > box.width * 1.35, 'selected portrait output does not expand into a full-width inspector')
  ...
  await check('Video prompt-first screen has no mandatory mode or testing context', async () => {
    ...
    await video.getByLabel('Video model').selectOption('mock')
    await video.getByLabel('Video prompt').fill('Handheld UGC shot of a woman holding a skincare product in a bright bedroom.')
    assert.equal(await page.getByText(/Red apple smoke validation|DISPOSABLE final Seedance/i).filter({ visible: true }).count(), 0)
    await assertBox(page, page.locator('.video-generator__rail'), box => box.width >= 270 && box.width <= 330, 'Video has a compact left control rail')
    await assertBox(page, page.locator('.video-generator__generate'), box => box.y < 740 && box.width > 150, 'Video Generate is visible without scrolling')
    await page.locator('.video-generator__generate').click()
    await confirmGeneration(page)
    await page.locator('.video-generator__results video').first().waitFor({ timeout: 20000 })
    await page.waitForFunction(() => {
      const video = document.querySelector('.video-generator__results video')
      return video && video.videoWidth > 0 && video.videoHeight > 0
    }, null, { timeout: 10000 })
    const videoAspect = await page.locator('.video-generator__results video').first().evaluate(video => ({ rendered: video.getBoundingClientRect().width / video.getBoundingClientRect().height, natural: video.videoWidth / video.videoHeight }))
  ...
  await check('Video ignores a stale project-linked quick pointer and opens a clean draft', async () => {
    ...
    const video = page.locator('.video-generator')
    await video.getByLabel('Video prompt', { exact: true }).waitFor()
    assert.equal(await video.getByLabel('Video prompt', { exact: true }).inputValue(), '')
    assert.equal(await page.locator('[data-testid^="generator-scene-"]').count(), 0)
    assert.equal(await page.getByText(/Red apple smoke validation|DISPOSABLE final Seedance/i).filter({ visible: true }).count(), 0)
    assert.equal(await page.getByText(/Quick draft|Link to project \/ testing context/i).filter({ visible: true }).count(), 0)
    await page.screenshot({ path: path.join(artifacts, 'video-stale-pointer-after.png'), fullPage: true })
  })

  await check('Video Auto creates a compact scene strip and selecting a scene updates its composer', async () => {
    await page.evaluate(() => sessionStorage.removeItem(`quick-generator:video:${localStorage.getItem('API_BASE_URL')}`))
    await page.reload()
    await clickNav(page, 'Video')
    await page.locator('.video-generator').getByLabel('Video prompt').fill('Create a 20-second UGC ad. Start with a strong hook, demonstrate the product, and finish with a natural CTA.')
    await page.locator('.video-generator__generate').click()
    await page.waitForFunction(() => document.querySelectorAll('.video-generator__scenes button').length >= 3)
    await page.locator('.video-generator__scenes button').nth(1).click()
    await page.locator('.video-generator').getByLabel('Video prompt').fill('Scene 2 edited: demonstrate the product close to camera with natural handheld movement.')
    assert.equal(await page.locator('[data-testid^="generator-scene-"]').count(), 0, 'Video does not fall back to giant stacked scene forms')
  ...
  await check('Studio hard reset: simple default, references, image/video results, templates, and advanced mode', async () => {
    await clickNav(page, 'Marketing Studio')
    await page.getByTestId('studio-workbench').waitFor()
    await page.getByRole('heading', { name: 'Marketing Studio', exact: true }).waitFor()
    await page.getByRole('button', { name: 'New', exact: true }).click()
    assert.equal(await page.getByLabel('Brief créatif Studio').inputValue(), '')
    assert.equal(await page.getByLabel('Studio media type').inputValue(), 'image')
    const forbidden = /Nom du produit|Faits produit|Langue du script|Product Test|Creative|Instruction visuelle|Script exact|Verrouiller ce script|Choisir une direction|Portrait produit|Rituel quotidien|Étude de matière|Un plan, une idée/
    assert.equal(await page.getByTestId('studio-workbench').getByText(forbidden).filter({ visible: true }).count(), 0)
    await assertBox(page, page.getByLabel('Brief créatif Studio'), box => box.y < 760 && box.height > 40, 'Studio prompt is visible above the fold')
    await assertBox(page, page.locator('.sw-generate'), box => box.y < 760 && box.width > 100, 'Studio Generate is visible above the fold')
    await page.screenshot({ path: path.join(artifacts, 'studio-default-desktop.png'), fullPage: true })
    await page.setViewportSize({ width: 1366, height: 768 })
    await assertBox(page, page.getByLabel('Brief créatif Studio'), box => box.y < 690 && box.height > 40, 'Studio prompt is visible at 1366x768 without scrolling')
    await assertBox(page, page.locator('.sw-generate'), box => box.y < 720, 'Studio Generate is visible at 1366x768 without scrolling')
    await page.screenshot({ path: path.join(artifacts, 'studio-default-1366.png'), fullPage: true })
    await page.setViewportSize({ width: 1440, height: 1000 })
    await page.getByLabel('Studio media type').selectOption('video')
    await page.getByLabel('Modèle Studio').selectOption('mock-video')
    await page.locator('.sw-attach-row summary[aria-label="Add Studio attachment"]').click()
    const refValue = await page.locator('select[aria-label="Photo produit Studio"] option').nth(1).getAttribute('value')
    assert.ok(refValue, 'Studio has a generated image Asset available for reference')
    await page.locator('select[aria-label="Photo produit Studio"]').selectOption(refValue)
    assert.ok(await page.locator('.sw-reference-pill', { hasText: 'Product' }).first().isVisible())
    await page.screenshot({ path: path.join(artifacts, 'studio-with-reference.png'), fullPage: true })
    await page.locator('.sw-attach-row summary[aria-label="Add Studio attachment"]').click()
    await page.getByRole('button', { name: 'New', exact: true }).click()
    ...
    assert.equal(await page.getByTestId('studio-workbench').getByText(/Scene 1|Scène 1|3 scènes|Instruction visuelle|Script exact/).filter({ visible: true }).count(), 0)
    ...
    await confirmGeneration(page)
    await page.locator('.sw-simple-card img').first().waitFor({ timeout: 20000 })
    await assertBox(page, page.locator('.sw-simple-card img').first(), box => box.width <= 360 && Math.abs(box.width - box.height) < 40, 'Studio image result stays aspect-correct in the simple gallery')
    await page.screenshot({ path: path.join(artifacts, 'studio-results-image.png'), fullPage: true })
    await page.getByRole('button', { name: 'New', exact: true }).click()
    await page.getByLabel('Brief créatif Studio').fill('Create a 15-second French UGC video for an eyebrow pen with before, demo and after.')
    await page.getByLabel('Studio media type').selectOption('video')
    await page.getByLabel('Modèle Studio').selectOption('mock-video')
    assert.equal(await page.getByTestId('studio-workbench').getByText(/Scene 1|Scène 1|3 scènes|Instruction visuelle|Script exact/).filter({ visible: true }).count(), 0)
    await page.locator('.sw-generate').click()
    ...
    assert.match(await dialog.innerText(), /Générer 3 clips?/)
    await dialog.getByRole('button', { name: 'Confirmer la génération', exact: true }).click()
    await page.locator('.sw-simple-card video').first().waitFor({ timeout: 60000 })
    await page.screenshot({ path: path.join(artifacts, 'studio-results-video.png'), fullPage: true })
    await page.getByRole('button', { name: 'Templates', exact: true }).click()
    await page.getByRole('dialog', { name: 'Studio Templates' }).locator('.sw-preset', { hasText: 'Portrait produit' }).waitFor()
    await page.getByRole('button', { name: 'Close', exact: true }).click()
    await page.getByRole('button', { name: 'Advanced Studio', exact: true }).click()
    await page.getByLabel('Nom du produit Studio').waitFor()
    await page.screenshot({ path: path.join(artifacts, 'studio-advanced.png'), fullPage: true })
    await page.getByRole('button', { name: 'Back to normal Studio', exact: true }).click()
    await page.getByRole('button', { name: 'New', exact: true }).click()
    assert.equal(await page.getByLabel('Brief créatif Studio').inputValue(), '')
    assert.equal(await page.getByTestId('studio-workbench').getByText(forbidden).filter({ visible: true }).count(), 0)
    await page.setViewportSize({ width: 390, height: 844 })
    await page.screenshot({ path: path.join(artifacts, 'studio-default-mobile.png'), fullPage: true })
    await page.setViewportSize({ width: 1440, height: 1000 })
